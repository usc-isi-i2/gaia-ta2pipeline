...
class GaiaRecord(rltk.AutoGeneratedRecord):
    ...
    def selected_target_index(self):
        if self.target:
            ...
            if len(selected_indices) == 1:
                ...
                target_name = [self.lower_labels(t) for t in self.target_name]
                idx = select_most_overlapped(self.name, *itemgetter(*selected_indices)(target_name))
                return selected_indices[idx]
    ...
    def selected_wikidata_labels(self):
        ...
        return self.lower_labels(ret)

    @rltk.cached_property
    def selected_target_labels(self):
        if self.selected_target:
            ret = self.target_name[self.selected_target_index]
            if ret:  # the ret could be None
                return self.lower_labels(ret)
        return []
    ...
    @rltk.cached_property
    def name(self):
        if self.raw_object['name']:
            return self.lower_labels(self.raw_object['name'])
        return []

    @staticmethod
    def lower_labels(l):
        l = filter(lambda x: isinstance(x, str), l)  # the value in list could be None, '' or np.nan.
...
class Cluster(object):
    ...
    def compute_confidence(self):
        ...
        for r in self.all_records:
            ...
            score = max(score, 0.01)  # lower bound, this mitigates the effect that round method makes score to be 0.0.
