...
class GaiaRecord(rltk.AutoGeneratedRecord):
    ...
    def selected_wikidata_index(self):
        if self.wikidata:
            ...
            if len(selected_indices) == 1:
                ...
                label_en = itemgetter(*selected_indices)(self.selected_wikidata_label_en)
                label_ru = itemgetter(*selected_indices)(self.selected_wikidata_label_ru)
                label_uk = itemgetter(*selected_indices)(self.selected_wikidata_label_uk)
                candidates = [list(label_en[i]) + list(label_ru[i]) + list(label_uk[i])
                              for i in range(len(selected_indices))]
                idx = select_most_overlapped(self.name, candidates)
    ...
    def selected_wikidata_labels(self):
        ...
        if self.selected_wikidata:
            ret += list(self.wikidata_label_en[self.selected_wikidata_index])
            ret += list(self.wikidata_label_ru[self.selected_wikidata_index])
            ret += list(self.wikidata_label_uk[self.selected_wikidata_index])
        return ret
    ...
    for bid, data in blocks_kb.key_set_adapter:
        ...
        for _, r_id in data:
            ...
            if r.target and not c.kb_id:
                ...
                c.kb_labels = r.selected_target_labels
            if r.wikidata:
                if r.selected_wikidata not in c.wd_candidate:
                    c.wd_candidate[r.selected_wikidata] = r.selected_wikidata_labels
            c.add(r)
        c.elect_wd_id()
