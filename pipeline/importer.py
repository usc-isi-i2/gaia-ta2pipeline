...
def exec_sh(s):
    logging.debug('exec_sh:', s)
    process = subprocess.Popen(s, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout, stderr = process.communicate()
    return stdout, stderr


def tmp_file_path(x=None):
    suffix = '' if not x else '.{}'.format(x)
    return os.path.join(config['temp_dir'], 'tmp{}'.format(suffix))


def clean_temp_files():
    for f in glob.glob(os.path.join(config['temp_dir'], 'tmp*')):
        os.remove(f)


def predicate_path(infile, path, retain_intermediate=False):
    all_p = path.split('/')
    if len(all_p) == 0:
        return
    ...
    exec_sh('kgtk filter -p "{tmp_str}" {infile} > {tmp_file}'
            .format(tmp_str=tmp_str, infile=infile, tmp_file=tmp_file_path()))
    pd_tmp1 = pd.read_csv(tmp_file_path(), delimiter='\t')
    ...
    for idx in range(1, len(all_p)):
        ...
        exec_sh('kgtk filter -p "{tmp_str}" {infile} > {tmp_file}'
                .format(tmp_str=tmp_str, infile=infile, tmp_file=tmp_file_path()))
        pd_tmp2 = pd.read_csv(tmp_file_path(), delimiter='\t')
...
def convert_nt_to_kgtk(nt_file, kgtk_file):
    logging.info('convert nt to kgtk')
...
def unreify_kgtk(infile, outfile):
    logging.info('unreify kgtk')
    exec_sh('kgtk unreify-rdf-statements -i {infile} / sort --columns 1,2 >  {outfile}'
            .format(infile=infile, outfile=outfile))


def create_entity_df(kgtk_file, unreified_kgtk_file, output_file, source, ldc_kg, df_wd_fb):
    logging.info('create entity df for ' + source)

    ### id
    logging.info('creating id')
    exec_sh('kgtk filter -p ";rdf:type;aida:Entity" {kgtk_file} > {tmp_file}'
            .format(kgtk_file=unreified_kgtk_file, tmp_file=tmp_file_path()))
    df_entity = pd.read_csv(tmp_file_path(), delimiter='\t')
    ...
    exec_sh('kgtk filter -p ";aida:hasName,aida:textValue;" {kgtk_file} > {tmp_file}'
            .format(kgtk_file=unreified_kgtk_file, tmp_file=tmp_file_path()))
    df_name = pd.read_csv(tmp_file_path(), delimiter='\t').drop(columns=['label']).rename(
    ...
    ### link target
    logging.info('creating link target')
    df_tmp1 = predicate_path(unreified_kgtk_file, 'aida:link/aida:linkTarget')\
        .rename(columns={'node1': 'e', 'node2': 'target'})
    # remove cluster
    df_target = pd.merge(df_entity, df_tmp1, left_on='e', right_on='e')
    # add confidence
    df_tmp2 = predicate_path(unreified_kgtk_file, 'aida:link/aida:confidence/aida:confidenceValue')\
    ...
    ### freebase id
    logging.info('creating freebase')
    exec_sh('kgtk filter -p ";aida:privateData,aida:jsonContent,aida:system;" {kgtk_file} > {tmp_file}'
            .format(kgtk_file=unreified_kgtk_file, tmp_file=tmp_file_path()))
    exec_sh('kgtk filter -p ";;rpi:EDL_Freebase" {tmp_file} > {tmp_file1}'
            .format(tmp_file=tmp_file_path(), tmp_file1=tmp_file_path(1)))
    exec_sh('kgtk ifexists --filter-on {tmp_file1} --input-keys node1 --filter-keys node1 -i {tmp_file} | kgtk filter -p ";aida:jsonContent;" > {tmp_file2}'
            .format(tmp_file1=tmp_file_path(1), tmp_file=tmp_file_path(), tmp_file2=tmp_file_path(2)))
    exec_sh('head -n 1 {kgtk_file} > {tmp_file3}'.format(kgtk_file=unreified_kgtk_file, tmp_file3=tmp_file_path(3)))
    exec_sh('kgtk filter -p ";aida:privateData;" {kgtk_file} | grep "entity:" >> {tmp_file3}'
            .format(kgtk_file=kgtk_file, tmp_file3=tmp_file_path(3)))
    df_tmp1 = pd.read_csv(tmp_file_path(3), delimiter='\t')
    df_tmp2 = pd.read_csv(tmp_file_path(2), delimiter='\t', quoting=csv.QUOTE_NONE, doublequote=False)
    ...
    ### embedding vector
    logging.info('creating embedding vector')
    exec_sh('kgtk filter -p ";aida:privateData,aida:jsonContent,aida:system;" {kgtk_file} > {tmp_file}'
            .format(kgtk_file=kgtk_file, tmp_file=tmp_file_path()))
    exec_sh('kgtk filter -p ";;rpi:entity_representations" {tmp_file} > {tmp_file1}'
            .format(tmp_file=tmp_file_path(), tmp_file1=tmp_file_path(1)))
...
-i {tmp_file} | kgtk filter -p ';aida:jsonContent;' > {tmp_file2}'''
    ...
    ### type
    logging.info('creating type')
    exec_sh('kgtk filter -p ";rdf:type;" {kgtk_file} | kgtk filter --invert -p ";;aida:Entity" > {tmp_file}'
            .format(kgtk_file=unreified_kgtk_file, tmp_file=tmp_file_path()))
    df_tmp1 = pd.read_csv(tmp_file_path(), delimiter='\t').rename(columns={'node1': 'e', 'node2': 'type'})
    ...
    ### wikidata
    logging.info('creating wikidata')
    ...
    logger.info('creating informative justification')
    df_infojust = predicate_path(unreified_kgtk_file,
    ...
    ### justified by
    logger.info('creating justified by')
    df_just = predicate_path(unreified_kgtk_file, 'aida:justifiedBy/aida:confidence/aida:confidenceValue',
                             retain_intermediate=True) \
    ...
    ### merge
    logger.info('merging all dfs to entity df')
    ...
    ### export
    if not validate_entity_df(df_entity_complete):
        logger.error('Invalid dataframe, please check input data')
    else:
        logger.info('exporting df')
...
def load_wd_to_fb_df():
    return pd.read_csv(config['wd_to_fb_file'])
...
def validate_entity_df(df):
    # for idx, r in df.iterrows():
    #     pass
    if len(df) != len(set(df['e'])):
        logger.error('Detected duplicate entities')
        return False
...
def process():
    ...
    for infile in glob.glob(os.path.join(config['input_dir'], '*.ttl.nt')):
        source = os.path.basename(infile).split('.')[0]
        kgtk_file = os.path.join(config['temp_dir'], '{}.ttl.nt'.format(source))
        unreified_kgtk_file = kgtk_file + '.unreified'
        output_file = os.path.join(config['temp_dir'], '{}.entity.h5'.format(source))
        # convert_nt_to_kgtk(infile, kgtk_file)
        # unreify_kgtk(kgtk_file, unreified_kgtk_file)
        create_entity_df(kgtk_file, unreified_kgtk_file, output_file, source, ldc_kg, df_wd_fb)
        # os.remove(kgtk_file)
